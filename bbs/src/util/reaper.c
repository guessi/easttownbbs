...
/* 保留帳號期限 -- 學期中 */
#define DAY_NEWUSR	7	/* 登入不超過三次的使用者保留   7 天(一周)   */
#define DAY_FORFUN	120	/* 未完成身分認證的使用者保留 120 天(四個月) */
#define DAY_OCCUPY	150	/* 已完成身分認證的使用者保留 150 天(五個月) */

/* 保留帳號期限 -- 暑假 */
#define VAC_NEWUSR	7	/* 登入不超過三次的使用者保留   7 天(一週)   */
#define VAC_FORFUN	180	/* 未完成身分認證的使用者保留 180 天(六個月) */
#define VAC_OCCUPY	210	/* 已完成身分認證的使用者保留 210 天(七個月) */

/* guessi.091203 已完成註冊者 多給30天寬限期 考量出國遊學 or 當兵的同學 */
  ...
  char *lowid;
{
  ...
  /* guessi.111009 為避免帳號占用狀況 板主若超過DAY_OCCUPY(VAC_OCCUPY)定義之時間未登入視為帳號棄置 拔除其板主權限 */
  if ((ulevel & PERM_BM) && (acct.lastlogin < due_occupy))
  {
    ulevel ^= PERM_BM;
    acct.userlevel = ulevel;
    lseek(fd, 0, SEEK_SET);
