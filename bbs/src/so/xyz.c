...
x_password()
{
  ...
  /* guessi.091225 清空畫面 避免使用者因畫面混亂而不知道在用什麼功能 */
  move(0, 0);
  vs_bar("查詢密碼");
  clrtobot();
  ...
  if (acct_get(msg_uid, &acct) > 0)
  {
    ...
    vget(b_lines - 2, 0, "請輸入該使用者認證時的 Email：", email, 40, DOECHO);
    ...
    vget(b_lines - 1, 0, "請輸入該使用者的真實姓名：", fpath, RNLEN + 1, DOECHO);
    if (strcmp(acct.realname, fpath))
    {
      vmsg("這不是該使用者註冊時填寫的真實姓名");
      return 0;
    }
    ...
    /* guessi.091225 增加[0-9]與[a-z]的可能 */
    /* 亂數產生 A~Z 組合的密碼八碼 */
    for (i = 0; i < PSWDLEN; i++)
    {
      if (rnd(2) == 0) {
    ...
    if (fp = fopen(fpath, "w"))
    {
      ...
      fprintf(fp, "※提示: 請注意大小寫\n");
      fclose(fp);

      bsmtp(fpath, BBSNAME "新密碼", email, 0);
      unlink(fpath);

      vmsg("新密碼已寄到該使用者的認證信箱");
    }
  }
