...
x_give()
{
  ...
  if (vget(21, 0, buf, fpath, 3, LCECHO) == 'y')
  {
    ...
    dollar -= dollar / 20 + ((dollar % 20 && !way) ? 1 : 0);	/* guessi.100103 收取 5% 手續費 金幣就算了 :p */
    ...
    sprintf(tbuf, "%s轉給您 共計 %d %s\n轉帳時刻: %s", cuser.userid, dollar, !way ? "銀" : "金", Btime(&now));
...
#define GOLD2MONEY	700000	/* 金幣→銀幣 匯率 */
#define MONEY2GOLD	770000	/* 銀幣→金幣 匯率 */
...
x_bank()
{
  ...
  outs("  ╠═════════════════════════════╣\n"
    "  ║ 目前銀行提供下列幾項服務：                               ║\n"
    "  ║\033[33m1.\033[37m 轉帳 -- 轉帳給其他人   (抽取 5% 手續費)  \033[36m              ║\n"
    "  ║\033[33m2.\033[37m 匯兌 -- 銀幣/金幣 兌換 (抽取 5% 手續費)  \033[36m              ║\n"
...
b_invis()
{
  ...
  else
  {
    ...
    {
      vmsg("請注意：切換隱身模式僅本次登入有效");

      if (vans("是否花 1 枚金幣切換為隱藏模式 (y/N) ") != 'y')
        return XEASY;

      if (cuser.gold < 1)
      {
        vmsg("很抱歉 金幣不足 切換隱身須扣除 1 枚金幣");
        return XEASY;
      }
      else
      {
