  ...
  NBRD *nbrd;
{
  ...
  len = b_cols + d_cols - strlen(nbrd->brdname) - 44; /* 前置 + 後墜 = 43 */

  /* 處理標題 */
  if ((nbrd->mode & NBRD_NEWBOARD) || (nbrd->mode & NBRD_DELBOARD))
    sprintf(title, "[%s][%-.*s]", nbrd->brdname, len, nbrd->title);
  else
    sprintf(title, "[%s]", nbrd->title);

  sprintf(class, "[\033[1;3%s\033[m]", (nbrd->mode & NBRD_NEWBOARD) ? "7m連署新板" :
                                       (nbrd->mode & NBRD_DELBOARD) ? "2m廢除看板" : "3m全民公投");

  /* 0001 12/02 userid      [連署類別] 看板名稱 標題 */
  prints("%6d %c %-5s %-13s %s%s\n", num, nbrd_attr(nbrd), nbrd->date + 3, nbrd->owner, class, title);
  /*                        ^ pos: 29 */
}
  ...
  XO *xo;
{
  ...
  {
    ans = vans("連署模式 1)連署新板 2)廢除看板 3)記名 4)無記名：[Q] ");
    if (ans < '1' || ans > '4')
  ...
  {
    ans = vans("連署模式 1)連署新板 2)廢除看板：[Q] ");
    if (ans < '1' || ans > '2')
      return xo->max ? XO_FOOT : nbrd_body(xo);
  }
  else
  {
    vmsg("對不起，本看板是唯讀的");
  ...
  if (ans == '1' || ans == '2')	/* 新板連署 or 連署廢除舊看板 */
  {
    if (!vget(b_lines, 0, "英文板名：", brdname, sizeof(nbrd.brdname), DOECHO))
      return XO_FOOT;

    if (ans == '1' && (brd_bno(brdname) >= 0 || !valid_brdname(brdname)))
    {
        vmsg("指定看板已存在，或名稱不合法");
        return XO_FOOT;
    }

    if (ans == '2' && (brd_bno(brdname) < 0 || !valid_brdname(brdname)))
    {
      vmsg("無法找到指定看板，或名稱不合法");
      return XO_FOOT;
    }
    ...
    {
      if (!vget(b_lines, 0, "看板主題：", title, sizeof(nbrd.title), DOECHO))
        return XO_FOOT;
    }
...
#ifdef SYSOP_START_COSIGN
    nbrd.mode = (ans == '1') ? NBRD_NEWBOARD : NBRD_DELBOARD;
#else
    nbrd.mode = (ans == '1') ? (NBRD_NEWBOARD | NBRD_START) : (NBRD_DELBOARD | NBRD_START);
#endif
  }
  else			/* 其他連署(站務發起的連署) */
  {
    ...
    if (!vget(b_lines, 0, "連署天數(1 ~ 30)：", tmp, 5, DOECHO))
      return XO_FOOT;
    days = atoi(tmp);
    if (days > 30 || days < 1)
      return XO_FOOT;
    if (!vget(b_lines, 0, "連署人數(1 ~ 500)：", tmp, 6, DOECHO))
      return XO_FOOT;
    numbers = atoi(tmp);
    if (numbers > 500 || numbers < 1)
      return XO_FOOT;

    nbrd.mode = (ans == '3') ? (NBRD_OTHER | NBRD_START) : (NBRD_OTHER | NBRD_START | NBRD_ANONYMOUS);
  }
  ...
  {
    fprintf(fp, "連署類型：\033[1;33m%s\033[m\n", (ans == '3') ? "記名公投" : "不記名公投");
    fprintf(fp, "連署主題：%s\n", title);
  }
  fprintf(fp, "舉辦日期：%s\n", nbrd.date);
  fprintf(fp, "到期天數：%d\n", days);
...
#ifdef SYSOP_START_COSIGN
  vmsg((ans == '1' || ans == '2') ? "送交申請了，請等候核准吧" : "連署開始了！");
#else
  vmsg("連署開始了！");
#endif
  return nbrd_init(xo);
  ...
  NBRD *hdd, *ram;
{
  if (hdd->total >= 0)
  {
    hdd->mode &= ~NBRD_END;
    hdd->mode &= ~NBRD_FINISH;
    hdd->mode |= NBRD_START; /* 無論是否達成人數 只要有反對 就拿掉完成標記 */
  }
  hdd->total += 1; /* 無論如何 反對 將所需人數加一 */
} /* guessi. 060414 加入反對功能 */
  ...
  XO *xo;
{
  ...
  *fname = 'G';
  ...
  if (!HAS_PERM(PERM_VALID)) /* 檢查認證 */
  {
    vmsg("通過認證才能參予連署唷！");
    return XO_FOOT;
  }

  if (!HAS_PERM(PERM_SYSOP) && nbrd_seek(fpath))
  {
    vmsg("您已經連署過了！");
    return XO_FOOT;
  }

  if (!HAS_PERM(PERM_SYSOP) && !strcmp(nbrd->owner, cuser.userid))
  {
    vmsg("本人不得參予連署");
    return XO_FOOT;
  } /* guessi.060414 連署發起人不得參與連署 */


  /* guessi.100912 提示使用者參與聯署將留下E-Mail資訊 */
  vmsg("請注意：本操作將會留下您註冊電子郵件信箱地址");
  ...
  *fname = '@';
  ...
  yn = vans("參與連署 (Y)贊成 (N)反對 (Q)跳出 [Q] "); /* 贊成 反對 */

  if ((yn == 'y' || yn == 'n') && vget(b_lines, 0, "我有話要說：", reason, 64, DOECHO))
  {
    FILE *fp;
...
*/
      ...
      {
        /* 反對顯示綠色 */
        fprintf(fp, "\033[1;32m[%3d]\033[m -> %s (%s)\n    %s\n",
        nbrd->total + 2, cuser.userid, cuser.email, reason);
	
	currchrono = nbrd->btime;
	rec_ref(xo->dir, nbrd, sizeof(NBRD), xo->pos, cmpbtime, disagree);
/*	guessi.090924 不做未讀判斷
        btime_update(brd_bno(BN_NEWBOARD));
*/
      }
    ...
    if (nbrd->total <= 1)
    {
      brd_fpath(folder, "AboutBoard", fn_dir);
      hdr_stamp(folder, HDR_COPY | 'A', &hdr, fpath);
      strcpy(hdr.owner, str_sysop);
      strcpy(hdr.nick, SYSOPNICK);
      sprintf(buf, "[記錄] %s看板連署項目完成", nbrd->brdname);
      strcpy(hdr.title, buf);
      hdr.xmode ^= POST_MARKED; /* guessi.060913 加上mark標記 */
      rec_bot(folder, &hdr, sizeof(HDR));

      btime_update(brd_bno("AboutBoard"));
      
      vmsg("連署人數達成");
    } /* guessi.060413 連署完成後貼文至看板 */
...
static int
nbrd_clear(xo) /* guessi.060609 消除標記 */
  XO *xo;
{
  NBRD *nbrd;
  ...
  XO *xo;
{
  ...
  brd_fpath(fpath, BN_NEWBOARD, fn_note);
