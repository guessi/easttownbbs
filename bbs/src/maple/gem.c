  ...
  int level;
{
  ...
  prints("%6d%c%c\241%c ", num, xmode & GEM_RESTRICT ? ')' : ' ', tag_char(hdr->chrono), gtype);

  /* guessi.060622 可見標題 但不可看內容 */
  if (gem_way == 0)
    prints("%.*s\n", d_cols + 64, hdr->title);
  else
  ...
  HDR *hdr;
{
  ...
  FILE *fp1, *fp2;
  char time_msg[20];
  /* guessi.060607 更動記錄上色 */
  char clr;

  if (hdr->xmode & GEM_RESTRICT) /* (GEM_RESTRICT | GEM_RESERVED)) */
    return;
  ...
  sprintf(time_msg, "%02d/%02d/%02d %02d:%02d:%02d",
  ptime->tm_year % 100, ptime->tm_mon + 1, ptime->tm_mday, ptime->tm_hour, ptime->tm_min, ptime->tm_sec);

  if (!(fp1 = fopen(fpath1, "a")))  /* @/@log */
    return;

  if (!strcmp(action, "新增"))
    clr = '3';
  else if (!strcmp(action, "修改"))
    clr = '2';
  else if (!strcmp(action, "刪除"))
    clr = '1';
  else if (!strcmp(action, "收錄"))
    clr = '6';
  else if (!strcmp(action, "標題"))
    clr = '5';
  else
    clr = '7';
  ...
  fprintf(fp1, "%s %12s [\033[1;3%cm%s\033[m] %-40.40s\n", time_msg, cuser.userid, clr, action, hdr->title);

  if (fp2 = fopen(fpath2, "r")) /* @/@log.old */
  {
    char buf[90];

    while (fgets(buf, 90, fp2))
    {
     if (strlen(buf) > 10)  /* 濾掉不必要的資訊 */
       fprintf(fp1, "%s", buf);
    }
    fclose(fp2);
  }
  ...
  XO *xo;
{
  ...
  if ((hdr->xmode & GEM_RESTRICT) && (hdr->xmode & GEM_RESERVED))
    return XO_NONE; /* GEM_RESERVED 若為隱藏 則不可修改 */
...
{
  /* 先比對板名，再比對分類 */
  int k = str_cmp(a->xname, b->xname);
  return k ? k : strncmp(a->title + BNLEN + 1, b->title + BNLEN + 1, BCLEN);
}
...
gem_sort(xo)
  XO *xo;
{
  if (!HAS_PERM(PERM_ALLBOARD))  /* 讓擁有ALLBOARD權限者 可排序精華區 */
    return XO_NONE;             /* 不開放一般板主使用 懶得寫比對是否為該板板主 */
                                 /* 避免板主修改其他板的精華區 051228 guessi */

  switch (vans("排序依據： 1) 分類->板名 2) 板名->分類 Q) 什麼都不做 [Q] "))
  {
    case '1':
      rec_sync(xo->dir, sizeof(HDR), hdr_cmp_class, NULL);
      break;
    case '2':
      rec_sync(xo->dir, sizeof(HDR), hdr_cmp_bname, NULL);
      break;
    default:
      return XO_FOOT;
  }
