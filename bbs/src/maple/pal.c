  ...
  int echo;
{
  ...
  vget(b_lines, 0, "友誼描述：", pal->ship, sizeof(pal->ship), echo);
#ifdef HAVE_BADPAL
  if (key != PALTYPE_VOTE)    /* 限制投票名單沒有壞人 */
    pal->ftype = vans("是否設為壞人(Y/N)？[N] ") == 'y' ? PAL_BAD : 0;
  else
#endif
    pal->ftype = 0;
}
...
#  ifdef HAVE_LIST
  fd = vans("要引入 (P)朋友名單 (B)板友名單 (1-5)特別名單？[Q] ");
#  else
  fd = vans("要引入 (P)朋友名單 (B)板友名單？[Q] ");
#  endif

#else

#  ifdef HAVE_LIST
  fd = vans("要引入 (P)朋友名單 (1-5)特別名單？[Q] ");
#  else
  fd = vans("要引入 (P)朋友名單？[Q] ");
#  endif
  ...
  char *userid;
{
  ...
  /* 若對方擋壞人的信，才檢查 */
  if (acct_load(&acct, userid) >= 0 && (acct.ufo & UFO_MAILBAD))
  {
