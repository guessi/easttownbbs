...
int ulist_userno[MAXACTIVE];           /* 對應 ushm 中各欄的 userno  */
...
/* itoc.020901: 用 cache 雖然比較好，但是蠻浪費記憶體的 */
typedef struct
{
  int userno;
  ...
  int sysop;
{
  time_t diff, ftype;
  int userno, ufo;
  char pager, buf[32], *fcolor, tmp[128];
  ...
  else
  {
    ...
    /* guessi.060320 簡化時間顯示 超過一分鐘才顯示閒置時間 */
    if ((diff = now - up->idle_time) >= 60)
    {
      if (diff % 60 > 30)
        sprintf(buf, "%3d'30", (int)(diff / 60));
      else
        sprintf(buf, "%3d'00", (int)(diff / 60));
    }
#else
...
# endif
#endif
  up->ufo & UFO_SUPERCLOAK ? "\033[1;35m" :
  ...
  if (pickup_ship)
  {
    sprintf(tmp, "%s", pal_ship(ftype, up->userno));
  }
  else
  {
#ifdef GUEST_WHERE
    /* 可看見 guest 的故鄉 */
    sprintf(tmp, "%s",(pager == ' ' || sysop || ftype & (FTYPE_SELF | FTYPE_BOTHGOOD | FTYPE_OGOOD) || !up->userlevel) ? (up->ufo & UFO_HIDFROM && !HAS_PERM(PERM_ALLADMIN)) ? "-" : up->from : "*");
#else
    /* 對方設我為好友可看見對方來源 */
    sprintf(tmp, "%s", (pager == ' ' || sysop || ftype & (FTYPE_SELF | FTYPE_BOTHGOOD | FTYPE_OGOOD) && up->userlevel) ? (up->ufo & UFO_HIDFROM && !HAS_PERM(PERM_ALLADMIN)) ? "-" : up->from : "*");
#endif
  ...
  prints("%6d%c%c%s%-13s%-*.*s\033[m%-*.*s%-11.10s\033[33m%-5.4s\033[m%s\n",
  num, ufo & UFO_CLOAK ? ')' : ' ', pager, fcolor, up->userid,
  (d_cols >> 1) + 19, (d_cols >> 1) + 18, up->username,
  d_cols - (d_cols >> 1) + 16, d_cols - (d_cols >> 1) + 15, tmp, bmode(up, 0), up->feeling, buf);
}
...
int pickup_way = 0;  /* 預設排列方式 0:代號 1:故鄉 2:動態 3:板伴 */
  ...
  XO *xo;
{
  ...
  /* itoc.010928: 由於 ushm->count 常不對，所以用 total_user 來校正，
     剛上站時就啟始化 total_user 為 ushm->count
     此後若使用者沒有來使用者名單，就不更新 total_user */
  if (!filter)
    total_user = slot;
  ...
  XO *xo;
{
  if (cuser.userlevel)
  {
    ...
    if (userno > 0 && (userno != cuser.userno) &&  /* lkchu.981217: 自己不可為朋友 */
        !is_mygood(userno) && !is_mybad(userno) && str_cmp(up->userid, STR_GUEST))    /* 尚未列入朋友名單 */
    {
      PAL pal;
      char fpath[64];
  ...
  XO *xo;
{
  ...
  sysop = sysop && !(cuser.ufo & UFO_PAL); /* 別改這段 !!! */
  ...
  {
    /* guessi.060420 廣播前詢問是否改用SYSOP帳號廣播 */
    if (sysop && vans("使用[sysop]帳號廣播？ Y)是 N)否 [N] ") == 'y')
    {
      bmw.sender = 0;
  ...
  XO *xo;
{
  ...
  if (vget(b_lines, 0, "◎踢除閒置多久的使用者(單位：分鐘)？ ", buf, 4, DOECHO))
    diff = atoi(buf);
  ...
  XO *xo;
{
  ...
  strcpy(buf, cutmp->feeling); /* 先copy 給自訂用 */

  vget(b_lines, 0, "自訂心情: ", buf, FLLEN + 1, GCARRY);

  strcpy(cuser.feeling, buf); /* copy 一份到ACCT */
  strcpy(cutmp->feeling, cuser.feeling); /* copy 至utmp */
  return ulist_body(xo); /* update */
}
#endif
