  ...
  char *mode, *id, *data;
{
  ...
  sprintf(buf, "%s %-10s %-13s %s\n", Now(), mode, id, data);
  f_cat(FN_RUN_REG, buf);
}
...
/* guessi.111113 提示站務人員尚有待審核註冊單 */
void
check_rform()
{
  if (HAS_PERM(PERM_ALLREG))
  {
    int rc = 0;
    rc = rec_num(FN_RUN_RFORM, sizeof(RFORM));
    if (rc > 5)
    {
      char msg[64];
      sprintf(msg, "請注意：目前有逾五筆認證註冊單等待審核");
      move(1, 0);
      clrtobot();
      vmsg(msg);
    }
...
acct_apply()
{
  ...
  vmsg("開始申請帳號前，請先詳細閱\讀站規！");

  /* guessi.060724 申請帳號時強迫閱讀站規 方能申請帳號 */
  if (more("gem/@/@et.rule", (char *) -1) >= 0)
  {
    if (vans("請問您是否同意以上站規？ Y)同意 N)不同意 [N] ") != 'y')
    {
      vmsg("歡迎再次光臨東方小城，再見。");
      abort_bbs();
    }
  ...
  for (;;)
  {
    vget(19, 0, "請設定密碼[至少四碼]：", buf, PSWDLEN + 1, NOECHO);
  ...
  /* guessi.060414 限制只能輸入男女 */
  for (;;)
  {
    /* 性別若非男女 則要求再輸入 */
    vget(22, 0, "性別 1)男性 2)女性 ", buf, 3, DOECHO);
  ...
  /* guessi.060414 資料填寫完畢 要求確認 */
  if (vans("請確認資料是否正確 Y)是 N)否 [N] ") != 'y')
  {
    vmsg("歡迎再次光臨" BBSNAME);
    abort_bbs();
  }
  ...
  int mode;
{
  ...
  {
    char nick[9][5] = {"遊子", "水滴", "訪客", "補帖", "鏢客", "影子", "路人", "俠客", "石像"};
    sprintf(cuser.username, "小城的%s", nick[ap_start % 9]);
  }
#endif    /* GUEST_NICK */
...
*/
    ...
    /* guessi.111007 不管如何一律記錄ip位置 */
    char name[16];
    sprintf(name, "%d.%d.%d.%d", addr[0], addr[1], addr[2], addr[3]);
    strcpy(utmp.from, name);
  }
#endif    /* HAVE_WHERE */

  /* guessi.101204 保存使用者登入IP位置(IPv4) */
  ...
  char *content;
{
  ...
  char passbuf[PSWDLEN + 1];
#endif
  double load[3]; /* 顯示系統load狀況 */

  getloadavg(load, 3); /* 顯示系統 load狀況 */

  move(b_lines, 0);

  /* guessi.051222 修改進站說明文字 並且顯示load狀況 */
  prints("   請輸入 \033[1;31m" STR_NEW "\033[m 註冊新帳號 或輸入 \033[1;32m" STR_GUEST "\033[m 入內參觀 城內人數：%-3d  " \
         "系統負載：\033[1;3%s\033[m", ushm->count, (load[0] > 20) ? "1m偏高": ((load[0] > 10) ? "3m中等" : "2m良好"));
...
#  endif
    ...
    else if (str_cmp(uid, STR_GUEST))    /* 一般使用者 */
    {
      ...
      else
      {
        ...
        uschar *addr;
        addr = (uschar *) &tn_addr;
        sprintf(from, "%d.%d.%d.%d", addr[0], addr[1], addr[2], addr[3]);

        /* guessi.120307 限制相同來源IP登入次數 */
        if (utmp_multi_login_count(from, 0) >= MAXIPLOGIN)
          login_abort("過多相同來源IP位置登入");
        ...
        if (!HAS_PERM(PERM_ALLADMIN))
        {
          ...
          pid = ui->pid;
          if (pid && vans("您想踢掉其他重複的login嗎？ Y)是 N)否 [Y] ") != 'n' && pid == ui->pid)
          {
    ...
    else /* guest */
    {
      ...
      /* guessi.101204 限制GUEST登入總數量 */
      if (utmp_count(cuser.userno, 0) >= MAXGUEST)
        login_abort("\n" STR_GUEST " 帳號已經爆滿了... 再見...");

      /* guessi.120307 限制相同來源IP登入次數 */
      if (utmp_multi_login_count(from, 0) >= MAXIPLOGINGUEST)
        login_abort("過多相同來源IP位置登入");
...
login_level()
{
  ...
  if (!(level & PERM_ALLADMIN))
  {
    ...
    level &= ~(PERM_10 | PERM_11 | PERM_12 | PERM_13 | PERM_14 | PERM_15 | PERM_16);
    /* 登入後先拿掉未定義的權限 新增權限時再作異動 070403 */
...
login_other()
{
  ...
  if (more(fpath,(char *) -1) >= 0 &&
      vans("以上為輸入密碼錯誤時的上站地點記錄，要刪除嗎(Y/N)？[N] ") == 'y')
    unlink(fpath);
...
tn_login()
{
  ...
  if (cuser.userlevel)        /* not guest */
  {
   ...
   prints("\033[1;37m★ 歡迎來到 \033[32m東華大學‧東方小城\033[37m 這是您第 \033[33m%d\033[37m 次光臨本站\033m\n" \
          "\033[1;37m☆ 上次您造訪本站時間為 \033[33m%s\033[m", cuser.numlogins, Btime(&cuser.lastlogin));
    ...
    if (cuser.money >= INT_MAX)
      cuser.money = INT_MAX - 1;

    if (cuser.gold >= INT_MAX)
      cuser.gold = INT_MAX - 1;
...
/* guessi.120615 short messages announcement while login */
static void
announce_out()
{
  FILE *fp;
  int len1, len2, count;
  char line1[SCR_WIDTH], line2[SCR_WIDTH];

  if (!(fp = fopen(FN_ETC_ANNOUNCE, "r")))
    return;
  ...
  {
    /* note: fgets would eat '\n' too */
    if (fgets(line2, SCR_WIDTH, fp) != NULL)
    {
      /* skip line with only '\n' or '\r' (empty line) */
      if (strlen(line2) <= 1)
        continue;
  ...
  if (count == 2)
  {
    /* note: str_ansi() will skip '\n' */
    str_ansi(line1, line1, strlen(line1));
    str_ansi(line2, line2, strlen(line2));

    /* length after removing ansi tags */
    len1 = strlen(line1);
    len2 = strlen(line2);

    /* here, we take 'count' as temp value */
    /* why 6 ? nothing special just for spacing and padding */
    if (len1 <= 10 && (count = SCR_WIDTH - len1 - len2 - 6) >= 0)
    {
      move(b_lines - 5, 0);
      clrtobot();
      prints("\n  \033[1;33;44m %s \033[m\033[0;30;47m %s %*s\033[m", line1, line2, count, "");
    }
  }
...
tn_main()
{
  ...
  film_out(FILM_BELOGIN, 0);

  if (load[0] > 30)
    login_abort("系統負載過高，請稍後再嘗試連線");
  ...
  time(&ap_start); /* guessi.060319 避免掛站時數暴增 */
