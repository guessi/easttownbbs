...
m_setforward()
{
  ...
  usr_fpath(fpath, cuser.userid, FN_AFORWARD);
  ...
  move(b_lines - 2, 0);
  prints("若要取消，請不要輸入任何文字");

  vget(b_lines - 1, 0, "請輸入信件自動轉寄的 E-mail：", ip, 50, GCARRY);

  if (ip[0] && !not_addr(ip) && vans("確定開啟自動轉信功\能(Y/N)？[N] ") == 'y')
  {
    if (fp = fopen(fpath, "w"))
    {
      fprintf(fp, "%s", ip);
  ...
  char *fpath, *userid, *title;
{
  ...
  usr_fpath(buf, userid, FN_AFORWARD);
  if (fp = fopen(buf, "r"))
  {
    fscanf(fp, "%s", buf);
    fclose(fp);
...
m_send()
{
  ...
  {
    if(strcmp(acct.userid, STR_GUEST)) /* 限制不可發信給guest 070103 guessi */
      my_send(acct.userid);
    else
      vmsg("不可以發信給guest唷！");
  }
  ...
  if ((fd = open(FN_ETC_SYSOP, O_RDONLY)) >= 0)
  {
	...
	*ptr = '\0';
    ...
    if (i >= 0 && i < j)
    {
      ...
      /* guessi.100423 若目前帳號為來賓帳號(guest) 給予提示訊息 */
      if (!str_cmp(cuser.userid, STR_GUEST))
      {
	prints("\n\n\n\033[1;31m請注意！！\033[m  您目前使用的帳號為\033[1;32m來賓帳號(guest)\033[m\n\n" \
	       "為加速信件處理速度，請您在信件中盡量詳述您所遭遇的問題\n" \
	       "並請務必於附上您的基本聯繫資料，以加速站務人員處理速度\n" \
	       "我們將在收件後盡快與您聯繫，謝謝您的合作！");
      }

      if (vget(b_lines - 11, 0, "信件標題：", ve_title, TTLEN + 1, DOECHO))
  ...
  HDR *hdr;
{
  ...
  xmode = hdr->xmode;
  prints(xmode & MAIL_DELETE ? "%6d%c\033[1;5;37;41m%c\033[m " : "%6d%c%s%c\033[m ",
    num, tag_char(hdr->chrono), (xmode & MAIL_MARKED) ? "\033[1;36m": "", mbox_attr(xmode));
#else
  prints("%6d%c%s%c\033[m ", num, tag_char(hdr->chrono), (hdr->xmode & MAIL_MARKED) ? "\033[1;36m" : "", mbox_attr(hdr->xmode));
#endif
...
{
  xo_load(xo, sizeof(HDR));
  MBoxKB = mbox_totalsize(xo->dir) >> 10;
  return mbox_head(xo);
}
  ...
  XO *xo;
{
  XO *xt;
  char *userid;
  char fpath[64], reason[72];

  /* 若已經進入第二人信箱，則不能再進第三人信箱 */
  if (xo != (XO *) &cmbox)
    return XO_NONE;

  /* itoc.030914: 讓所有 ADMINs 都可以看 sysop/guest 信箱 */
  if (!HAS_PERM(PERM_ALLADMIN))
    return XO_NONE;

  switch (vans("進入 (1) SYSOP信箱 2) GUEST信箱？[Q] "))
  {
  case '1':
    userid = str_sysop;
    break;
  ...
  /* guessi.060421 進入信箱詢問原因 */      
  if (!vget(b_lines, 0, "原因: ", fpath, 65, DOECHO))
    return XO_FOOT;

  sprintf (reason, "查看 %s 信件 原因: %s", userid, fpath);
  alog("進入信箱", reason);
