...
void
check_user_bm(userid, userbm)    /* guessi.090611 回傳user擔任哪些板主 */
  char *userid;
  char *userbm;
{
  BRD *bhdr, *tail;
  ...
  do
  {
    if (is_bm(bhdr->BM, userid))    /* 排版 */
      sprintf(userbm, "%s%s%s", userbm, (strlen(userbm) > 0) ? " " : "", bhdr->brdname);
  } while (++bhdr < tail);
}
  ...
  usint ulevel;
{
  ...
  */
  ...
  if ((readlevel == PERM_ALLADMIN) && is_bgood(bpal)) /* guessi.060613 為小城開的 舊站長可進入ADMIN看板 */
    bits |= BRD_W_BIT; /* 只給BRD_W_BIT 其餘由其他函式判斷 這邊不做判斷 L R */
...
static int class_inn = 0;    /* guessi.090907 標記轉信看板 */
  ...
  XO *xo;
{
  ...
  do
  {
    ...
    if (chn >= 0)    /* 一般看板 */
    {
      ...
      /* guessi.111021 無論如何 強制將公告看板置入熱門 */
      if (class_hot && (strcmp((brd+chn)->brdname, BN_ANNOUNCE) && bshm->mantime[chn] < 5))
        continue;

      /* guessi.090907 建立轉信看板列表 */
      if (class_inn && (brd[chn].battr & BRD_NOTRAN))
        continue;
    }
    else        /* 分類群組 */
    {
  ...
  {
    cbase -= bnum;
    qsort(cbase, bnum, sizeof(short), mantime_cmp);
  }
  ...
  char *folder;
{
  ...
  if ((fd = open(folder, O_RDONLY)) >= 0)
  {
    ...
    {
      if (!(nbrd.mode & NBRD_FINISH) && !(nbrd.mode & NBRD_END))
      {
        rc = 1;
        break;
      }
  ...
  int num, bno, brdpost;
{
  ...
  if (brd->bvote)
   ...
   bno = bshm->mantime[bno];  // bno =  utmp_brdmate(brd->brdname);
  ...
  if (bno > 599)
    ...
    sprintf(str3 = buf, "\033[1;31m%3d\033[m", bno);
  else if (bno > 99)
    sprintf(str3 = buf, "\033[1;32m%3d\033[m", bno); /* guessi.060528 三位數字特別處理 */
  else if (bno > 79)
    sprintf(str3 = buf, " \033[1;32m%2d\033[m", bno);
  else if (bno > 49)
    sprintf(str3 = buf, " \033[1;33m%2d\033[m", bno);
  else if (bno > 0)
  ...
  prints("%6d%c%s%-13s\033[;3%dm%-5s\033[m%s %-*.*s%s %.*s\n",
    num, token, str1, brd->brdname,
    (brd->class[3] & 5) + 2, brd->class, str2,
  ...
  do
  {
    ...
    if (cnt < max)
    {
      ...
      else            /* 分類群組 */
      {
        ...
        chx = (short *) img + (CH_END - chn);
        str = img + *chx;
        prints("%6d%c。%-13.13s\033[3%dm%-5.5s\033[m%s\n",
  ...
  XO *xo;
{
  ...
  if (!cuser.userlevel)
    return XO_NONE;  /* 不給guest使用 060701 guessi */
  ...
  XO *xo;
{
  ...
  if (chn < 0)        /* 進入分類 */
  {
    ...
    if (!strncmp(str, "HOTBOARD/", 9))
    {
      class_hot = 1;
      chn = CH_END;
    }
    /* guessi.090907 進入指定分類:轉信看板 */
    if (!strncmp(str, "TRANSBRD/", 9))
    {
      class_inn = 1;
      chn = CH_END;
    }
  ...
  XO *xo;
{
  ...
  if (chn >= 0)
  {
    ...
    {
      vmsg("此看板已在我的最愛 如需新增請至我的最愛加入");
    }
  }
  else /* LHD.051007: 看板分類加入我的最愛 */
  {
    short *chx;
    char *img, *str, *ptr;
