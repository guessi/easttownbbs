  ...
  int header_len;
{
  ...
  for (i = 1; i < LINE_HEADER; i++)
  {
        ...
        (header_len == LEN_AUTHOR2 && !memcmp(str, header2[i], LEN_AUTHOR2 - 1)))
    {
      ...
      word = str + header_len;
      if (vmode)
        prints("\033[1;34m│ \033[36m%s\033[37m%-*.*s\033[34m │\033[m", header1[i], d_cols + 68, d_cols + 68, word);
      else
        prints("│ %s%-*.*s │", header1[i], d_cols + 68, d_cols + 68, word);
      return;
    }
  }
...
more_slideshow()
{
  ...
  if (!slideshow)
  {
    ...
    if (ch == '@')
    {
      slideshow = vans("請選擇放映的速度 1(最慢)～9(最快) [預設: 5] ") - '0';
