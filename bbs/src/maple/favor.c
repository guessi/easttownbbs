  ...
  MF *mf;
{
  ...
  if (mftype & MF_FOLDER)
  {
    ...
    prints("%6d%c。FavorFolder  目錄 %s %s\n", num, mftype & MF_MARK ? ')' : ' ', "◇", mf->title);
  }
  ...
  else if (mftype & MF_CLASS)
  {
    char cname[BNLEN + 2];
    sprintf(cname, "%s/%-*s", mf->xname, 12 - strlen(mf->xname), "");
    prints("%6d 。%s\033[3%dm%-5.5s\033[m%s\n", num, cname, mf->class[3] & 7, mf->class, mf->title);
  }
  ...
  XO *xo;
{
  ...
  prints(NECKER_MF, cuser.ufo & UFO_BRDPOST ? "總數" : "編號", d_cols >> 1, "", d_cols - (d_cols >> 1), "");
  return mf_body(xo);
}
  ...
  XO *xo;
{
  ...
  {
    if (ans == 'f' && !vget(b_lines, 0, "標題：", mf.title, BTLEN + 1, DOECHO))
      return XO_FOOT;
  ...
  if (nb == 2 || nb == 3)   /* 看板、精華區捷徑 */
  {
    ...
    if (na >= 0)
    {
      if (nb >= 0)
      {
        ...
        return nb ? nb : strcmp(bshm->bcache[na].title, bshm->bcache[nb].title);
      }
      return -1;
    }
...
mf_sort(xo)
  XO *xo;
{
  if (vans("是否切換我的最愛排序方式(板名/分類)？ Y)切換 N)離開 [N] ") != 'y')
    return XO_FOOT;
  ...
  XO *xo;
{
  ...
  switch (vans("請選擇你要的功\能： 1)備份我的最愛 2)還原我的最愛？[Q] "))
  {
     case '1':
