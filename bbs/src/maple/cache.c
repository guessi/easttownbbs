  ...
  char *userid;
{
  ...
  {
    /* 已經離站的不檢查 */
    if (uentp->pid && ((userno && uentp->userno == userno) || (userid && !strcmp(userid, uentp->userid))))
    {
...
int
utmp_multi_login_count(from, show)
  char *from;
  int show;
{
  UTMP *uentp, *uceil;
  ...
  do
  {
    if (!strcmp(uentp->from, from))
    {
      count++;
      if (show)
      {
        prints("(%d) %s -> %s\n",
        count, uentp->userid, uentp->from);
      }
    }
  } while (++uentp <= uceil);
  return count;
