...
typedef struct
{
  time_t stamp;         /* 資料建立日期時間 */
  int type;             /* 資料類別 */
  time_t chrono;        /* 使用者自訂之資料日期時間 */
  char flag;            /* 支出/收入 */
  int money;            /* 金額 */
  char useway;          /* 分類(食衣住行育樂) */
  char desc[75];        /* 說明 */
  char desc_way[5];     /* 分類自訂 */
}      CREDIT;

#define CREDIT_FOLDER   0x01    /* 資料夾 */
#define CREDIT_DATA     0x02    /* 一般資料 */
...
#define NBRD_DELBOARD	0x40000 /* 廢除看板 */
...
#define FILM_BELOGIN	3	/* 起始畫面(短暫) */
#define FILM_GOODBYE	4	/* 再見畫面 */
#define FILM_NOTIFY	5	/* 尚未通過認證通知 */
#define FILM_MQUOTA	6	/* 信件超過保存期限通知 */
#define FILM_MAILOVER	7	/* 信件封數過多通知 */
#define FILM_MGEMOVER	8	/* 個人精華區過多通知 */
#define FILM_BIRTHDAY	9	/* 生日當天的上站畫面 */
#define FILM_APPLY	10	/* 註冊提示畫面 */
#define FILM_JUSTIFY	11	/* 身份認證的方法 */
#define FILM_REREG	12	/* 重新認證說明 */
#define FILM_EMAIL	13	/* 郵件信箱認證說明 */
#define FILM_NEWUSER	14	/* 新手上路須知 */
#define FILM_TRYOUT	15	/* 密碼錯誤 */
#define FILM_POST	16	/* 文章發表綱領 */
